...
/// folding this into AiSuggestMenuBarApp) so the parsing logic has a
...
final class OverlayBridge {
    ...
    private func waitAndDump(matching namePart: String, deadline: Date) {
        ...
        if name.contains(namePart) || bundleId.contains(namePart) {
            debugLog("ai-suggest-menubar: [ax-dump] matched \"\(namePart)\" -> "
                + "\(frontmost?.localizedName ?? "?"), dumping now")
            TerminalPositioner.dumpAXTreeForFrontmostApp()
            return
        }
        guard Date() < deadline else {
            debugLog("ai-suggest-menubar: [ax-dump] timed out waiting for \"\(namePart)\" to become frontmost "
...
struct AiSuggestMenuBarApp: App {
    ...
    init() {
        ...
        debugLog("ai-suggest-menubar: AXIsProcessTrustedWithOptions(prompt)=\(trusted)")
    }
