...
final class OverlayServer {
    ...
    func start() {
        ...
        guard fd >= 0 else {
            debugLog("ai-suggest-menubar: overlay socket() failed: \(String(cString: strerror(errno)))")
            return
        }
        ...
        guard listen(fd, 8) == 0 else {
            debugLog("ai-suggest-menubar: overlay listen() failed: \(String(cString: strerror(errno)))")
            close(fd)
            return
        }
    ...
    private func bindUnixSocket(fd: Int32, path: String) -> Bool {
        ...
        guard pathBytes.count <= maxLen else {
            debugLog("ai-suggest-menubar: overlay socket path too long: \(path)")
            return false
        }
        ...
        guard result == 0 else {
            debugLog("ai-suggest-menubar: overlay bind() failed: \(String(cString: strerror(errno)))")
            return false
        }
        return true
    }
