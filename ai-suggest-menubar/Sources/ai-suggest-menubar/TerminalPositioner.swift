...
struct PositionerConfig: Decodable {
    ...
    static let shared: PositionerConfig = {
        ...
        guard let data = try? Data(contentsOf: path) else {
            debugLog("ai-suggest-menubar: no overlay_position.json at \(path.path), using built-in defaults")
            return PositionerConfig()
        }
        do {
            let decoded = try JSONDecoder().decode(PositionerConfig.self, from: data)
            debugLog("ai-suggest-menubar: loaded overlay_position.json override from \(path.path)")
            return decoded
        } catch {
            debugLog("ai-suggest-menubar: failed to parse overlay_position.json: \(error) — using built-in defaults")
            return PositionerConfig()
        }
    }()
}
...
enum TerminalPositioner {
    ...
    static func anchor(for cursor: CursorInfo) -> ScreenAnchor? {
        guard let app = NSWorkspace.shared.frontmostApplication else {
            debugLog("ai-suggest-menubar: overlay position: no frontmost app")
            return nil
        }
        ...
        if let focusedEl = focusedElement(pid: app.processIdentifier),
           let rect = caretRect(for: focusedEl) {
            ...
            debugLog("ai-suggest-menubar: overlay position: caret bounds for \(app.localizedName ?? "?") "
        ...
        guard !candidates.isEmpty else {
            debugLog("ai-suggest-menubar: overlay position: no focused window/element for "
        ...
        for candidate in candidates {
            ...
            guard cellWidthRange.contains(cellWidth), cellHeightRange.contains(cellHeight) else {
                debugLog("ai-suggest-menubar: overlay position: rejecting \(candidate.label) for "
            ...
            debugLog("ai-suggest-menubar: overlay position: using \(candidate.label) for "
        ...
        debugLog("ai-suggest-menubar: overlay position: no plausible geometry for \(app.localizedName ?? "?")")
        return nil
    }
    ...
    private static func focusedElement(pid: pid_t) -> AXUIElement? {
        ...
        debugLog("ai-suggest-menubar: kAXFocusedUIElementAttribute error=\(error.rawValue)")
        ...
        guard attribute(element, kAXPositionAttribute, as: CGPoint.self, axType: .cgPoint) != nil,
            ...
            debugLog("ai-suggest-menubar: focused UI element has no position/size, falling back to window")
            return nil
        }
        return element
    }
    ...
    private static func focusedWindow(pid: pid_t) -> AXUIElement? {
        debugLog("ai-suggest-menubar: AXIsProcessTrusted=\(AXIsProcessTrusted())")
        let appElement = AXUIElementCreateApplication(pid)

        var windowRef: CFTypeRef?
        var error = AXUIElementCopyAttributeValue(appElement, kAXFocusedWindowAttribute as CFString, &windowRef)
        debugLog("ai-suggest-menubar: kAXFocusedWindowAttribute error=\(error.rawValue)")
        ...
        debugLog("ai-suggest-menubar: kAXMainWindowAttribute error=\(error.rawValue)")
        if error == .success, let windowRef {
            // swiftlint:disable:next force_cast
            return (windowRef as! AXUIElement)
        }
        ...
        debugLog("ai-suggest-menubar: kAXWindowsAttribute error=\(error.rawValue) "
    ...
    static func dumpAXTreeForFrontmostApp() {
        guard let app = NSWorkspace.shared.frontmostApplication else {
            debugLog("ai-suggest-menubar: [ax-dump] no frontmost app")
            return
        }
        debugLog("ai-suggest-menubar: [ax-dump] === \(app.localizedName ?? "?") "
        ...
        debugLog("ai-suggest-menubar: [ax-dump] === done ===")
    }
