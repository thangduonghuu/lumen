...
| **Smart `rm` cleanup suggestions** | Typing `rm -rf` (or `-r`/`-f`) suggests build artifacts, dependencies, and caches to delete — `node_modules`, `.build`, `dist`, `__pycache__`, and 40+ more — but only ones actually present in the current directory, not the full hypothetical list. |
